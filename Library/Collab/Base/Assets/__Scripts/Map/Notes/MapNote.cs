...
[System.Serializable]
public class MapNote : BeatmapObject {
    ...
    public const int NOTE_TYPE_BOMB = 2;
    //public const int NOTE_TYPE_GHOST = 3;
    ...
    public MapNote(JSONNode node) {
        ...
        beatmapType = (_type == 3) ? Type.Bomb : Type.Note;
    }

    public MapNote(float time, int lineIndex, int lineLayer, int type, int cutDirection) {
        _time = time;
        _lineIndex = lineIndex;
        _lineLayer = lineLayer;
        _type = type;
        _cutDirection = cutDirection;
        beatmapType = (type == 3) ? Type.Bomb : Type.Note;
    }
    ...
    public override float _time { get; }
