...
{
    // BLOOM_FOG selects the bloom prepass sample. Blue-noise dithering follows this branch in every route.
    // Noise coordinates include object translation. Bloom UV coordinates remain unchanged.
    ...
    || defined(UNITY_STEREO_MULTIVIEW_ENABLED)
            ...
            v2f vert(appdata v)
            {
                ...
                float2 normalizedPosition = ComputeNonStereoScreenPos(o.vertex).xy;
                #if defined(UNITY_SINGLE_PASS_STEREO) || defined(STEREO_INSTANCING_ON) \
    || defined(UNITY_STEREO_MULTIVIEW_ENABLED)
