...
{
    // LIV_MR cancels CLEAR_SCREEN_ALPHA in both eyes.
    // CLEAR_SCREEN_ALPHA writes output alpha 1.0, not 0, without changing RGB composition.
    // The blue-noise lookup is (uv + (0.1, 0.2)) * params + random.
    // When source texel-size y is negative, the vertex flips the second UV's y component.
    // Main taps use v1.xy. The bloom tap uses v1.zw with that conditional flip.
    // The ChroMapper blit path uses untransformed quad UVs, matching the source chain's identity transform.
    // Main and bloom taps use zero sample bias because no ChroMapper producer supplies a bias value.
    // The stereo route samples Texture2DArray sources by eye index and renders per-eye instances.
    // Blue noise remains a plain Texture2D in both routes.
    // Composition order is bloom*intensity + dither, plus scene, times fade.
    // Outside the clear route, output alpha is the center-tap alpha. Fade never multiplies output alpha.
