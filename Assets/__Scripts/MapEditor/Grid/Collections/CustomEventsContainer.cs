...
public class CustomEventsContainer : BeatmapObjectContainerCollection, CMInput.ICustomEventsContainerActions
{
    ...
    public override BeatmapObject.ObjectType ContainerType => BeatmapObject.ObjectType.CustomEvent;
    ...
    public override IEnumerable<BeatmapObject> GrabSortedObjects() =>
        UnsortedObjects.OrderBy(x => x.Time).ThenBy(x => (x as BeatmapCustomEvent).Type);
    ...
    private void HandleTrackAssign(string res)
    {
        ...
        foreach (var obj in SelectionController.SelectedObjects) obj.GetOrCreateCustomData()["_track"] = res;
    }

    public override BeatmapObjectContainer CreateContainer() =>
        BeatmapCustomEventContainer.SpawnCustomEvent(null, this, ref customEventPrefab);
}
