...
public class BPMChangeGridContainer : BeatmapObjectContainerCollection
{
    ...
    private void CreateAutogeneratedBpmChange(int res)
    {
        ...
        IBpmChange autoGenerated = BeatSaberSongContainer.Instance.Map.GetVersion() == 3
            ? (IBpmChange)new V3BpmChange(
            ...
            : new V2BpmChange(BeatSaberSongContainer.Instance.Song.BeatsPerMinute,
                AudioTimeSyncController.GetBeatFromSeconds(offset / 1000f)) {CustomData = new JSONObject() };
    ...
    {
        IBpmEvent lastChange = null;

        foreach (var obj in LoadedObjects)
        {
            var bpmChange = obj as IBpmEvent;
    ...
    public void RefreshGridProperties()
    {
        ...
        if (LoadedContainers.Count > 0)
        {
            ...
            foreach (var bpmChangeKvp in activeBpmChanges)
            {
                ...
                var bpmChange = bpmChangeKvp.Key as IBpmEvent;
    ...
    /// <summary>
    ///     Find the last <see cref="IBpmEvent" /> before a given beat time.
    /// </summary>
    /// <param name="beatTimeInSongBpm">Time in raw beats (Unmodified by any BPM Changes)</param>
    /// <param name="inclusive">Whether or not to include <see cref="IBpmEvent" />s with the same time value.</param>
    /// <returns>The last <see cref="IBpmEvent" /> before the given beat (or <see cref="null" /> if there is none).</returns>
    public IBpmEvent FindLastBpm(float beatTimeInSongBpm, bool inclusive = true)
    {
        if (inclusive)
            return LoadedObjects.LastOrDefault(x => x.Time <= beatTimeInSongBpm + 0.01f) as IBpmEvent;
        return LoadedObjects.LastOrDefault(x => x.Time + 0.01f < beatTimeInSongBpm) as IBpmEvent;
    }

    /// <summary>
    ///     Find the next <see cref="IBpmEvent" /> after a given beat time.
    /// </summary>
    /// <param name="beatTimeInSongBpm">Time in raw beats (Unmodified by any BPM Changes)</param>
    /// <param name="inclusive">Whether or not to include <see cref="IBpmEvent" />s with the same time value.</param>
    /// <returns>The next <see cref="IBpmEvent" /> after the given beat (or <see cref="null" /> if there is none).</returns>
    public IBpmEvent FindNextBpm(float beatTimeInSongBpm, bool inclusive = false)
    {
        if (inclusive)
            return LoadedObjects.FirstOrDefault(x => x.Time >= beatTimeInSongBpm - 0.01f) as IBpmEvent;
        return LoadedObjects.FirstOrDefault(x => x.Time - 0.01f > beatTimeInSongBpm) as IBpmEvent;
    }
